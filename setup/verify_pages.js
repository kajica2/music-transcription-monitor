...
const URL = 'https://kajica2.github.io/music-transcription-monitor';
...
(async () => {
  ...
  const pages = [
    ...
    { path: '/tester.html',       name: 'tester', expects: ['Solo Instrument Transcription Tester', 'ACF2+', 'Sample melodies', 'Debug log', 'C major scale'] },
  ];
  for (const p of pages) {
    ...
    console.log(`  ${status === 200 && missing.length === 0 ? '✓' : '✗'} ${p.name.padEnd(8)} status=${status} missing=${missing.length ? missing.join(',') : 'none'}`);
    await page.close();
  }
  await browser.close();
  console.log('');
  ...
  console.log('  ✓ No runtime errors. GitHub Pages deployment verified.');
})();
